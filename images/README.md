...
## Homepage hero video (the opening montage)

| File to add | Goes in folder | Spec |
|---|---|---|
| `montage.mp4` | `images/hero/` | H.264, 1920×1080, 15–25s, silent (it autoplays muted) |
| `montage.webm` | `images/hero/` | Same shot as above, VP9/AV1 — smaller file for Chrome/Firefox/Edge |
| `poster.jpg` | `images/hero/` | One still frame from the video, 1920×1080 — shown instantly while the video loads |

Activation is the same pattern: open `index.html`, find the commented `<source>` lines in the `<video>` tag near the top (search for "hero montage"), uncomment them, and add `poster="images/hero/poster.jpg"` as an attribute on the `<video>` tag itself. Until you do, the hero falls back to the ink-green gradient background that's already there — nothing looks broken in the meantime.

No identifiable patient in frame, same as the photos above.
